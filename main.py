...
"""
        ...
        # Extract JSON block from Grok response
        json_match = re.search(r'```json\n(.*)\n```', grok_content, re.DOTALL)
        if json_match:
            grok_data = json_match.group(1)
        else:
            logger.error(f"Failed to extract JSON from Grok response: {grok_content}")
        ...
        story.append(Paragraph(f"<b>Uploaded</b>: {os.getenv('CURRENT_DATE', '2025-07-19 16:15:00 CDT')}", styles['Normal']))  # Updated to current time
