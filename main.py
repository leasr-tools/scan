...
async def process_lease(request: Request):
    ...
    try:
        ...
        # Extract email from Tally metadata (assuming it's the last part before extra data)
...
"""
        # Claude API call with detailed logging
        logger.info("Calling Claude API with prompt length: {len(claude_prompt)} and text length: {len(text)}")
...
"""
        ...
        report_content = (
            ...
            f"**Uploaded**: {os.getenv('CURRENT_DATE', '2025-07-17 22:11:45 CDT')}\n"
        ...
        logger.error(f"Claude response details: status={claude_response.status_code}, text={claude_response.text if 'claude_response' in locals() else 'N/A'}")
        return {
            "status": "error",
